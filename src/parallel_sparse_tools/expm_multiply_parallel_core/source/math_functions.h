...
inline npy_cdouble_wrapper exp(const npy_cdouble_wrapper &value) {
  std::complex<double> res =
      std::exp(std::complex<double>(value.real, value.imag));
  return npy_cdouble_wrapper(res.real(), res.imag());
}

inline npy_cfloat_wrapper exp(const npy_cfloat_wrapper &value) {
  std::complex<float> res =
      std::exp(std::complex<float>(value.real, value.imag));
  return npy_cfloat_wrapper(res.real(), res.imag());
}

inline double exp(const double &value) { return std::exp(value); }
...
inline void print(const double value) { printf("%f\n", value); }

inline void print(const float value) { printf("%f\n", value); }
