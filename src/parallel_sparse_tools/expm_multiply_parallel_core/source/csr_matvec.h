...
#pragma message(                                                               \
    "This header file is deprecated, this should only be used for csr_matvec_wrapper.pyx")
  ...
  for (int tid = 0; tid < num_threads; tid++) {
    ...
    for (; thread_coord.x < thread_coord_end.x; ++thread_coord.x) {
      for (; thread_coord.y < row_end_offsets[thread_coord.x]; ++thread_coord.y)
        running_total +=
            T3(values[thread_coord.y]) * x[column_indices[thread_coord.y]];
...
void inline csr_matvec(const bool overwrite_y, const I n, const I Ap[],
                       const I Aj[], const T1 Ax[], const T2 a, const T3 x[],
                       I rco[], T3 vco[], T3 y[]) {
...
void csr_matvec(const bool overwrite_y, const I n, const I Ap[], const I Aj[],
                const T1 Ax[], const T2 a, const T3 x[], I rco[], T3 vco[],
                T3 y[]) {
  if (overwrite_y) {
    for (I k = 0; k < n; k++) {
      T3 sum = 0;
