...
template <typename I, typename T1, typename T2, typename T3>
void csr_matvec_multi(const bool overwrite_y, const I n_row,
                      const npy_intp n_vecs, const I Ap[], const I Aj[],
                      const T1 Ax[], const T2 a, const T3 x[], I rco[],
                      T3 vco[], T3 y[]) {
  T3 *y_row = y;
  if (overwrite_y) {
...
template <typename I, typename T1, typename T2, typename T3>
void expm_multiply(const I n, const I Ap[], const I Aj[], const T1 Ax[],
                   const int s, const int m_star, const T2 tol, const T3 mu,
                   const T3 a, T3 F[], T3 work[]) {
...
#pragma omp parallel shared(exit_loop, c1_threads, c2_threads, c3_threads, F,  \
                                B1, B2, rco, vco) firstprivate(num_threads)
  {
...
#if defined(_OPENMP)
        csrmv_merge<I, T1, T3, T3>(true, n, Ap, Aj, Ax, a / T2(j * s), B1, rco,
                                   vco, B2); // implied barrier
#else
        csr_matvec<I, T1, T3, T3>(true, n, Ap, Aj, Ax, a / T2(j * s), B1, rco,
                                  vco, B2);
#endif
...
template <typename I, typename T1, typename T2, typename T3>
void expm_multiply_batch(const I n, const npy_intp n_vecs, const I Ap[],
                         const I Aj[], const T1 Ax[], const int s,
                         const int m_star, const T2 tol, const T3 mu,
                         const T3 a, T3 F[], T3 work[]) {
...
#if defined(_OPENMP)
        csrmv_merge_multi<I, T1, T3, T3>(true, n, n_vecs, Ap, Aj, Ax,
                                         a / T2(j * s), B1, rco, vco,
                                         B2); // implied barrier
#else
        csr_matvec_multi<I, T1, T3, T3>(true, n, n_vecs, Ap, Aj, Ax,
                                        a / T2(j * s), B1, rco, vco, B2);
#endif
