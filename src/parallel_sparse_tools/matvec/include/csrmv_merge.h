...
// See work my Merrill et. al.
// (http://ieeexplore.ieee.org/abstract/document/7877136/) for original work and
// implementation. This code contains modified versions of algorithms 2 and 3.

template <class I> class CountingInputIterator {
  const I init;
...
template <class I> struct CoordinateT {
  I x, y;
  CoordinateT(I _x, I _y) : x(_x), y(_y) {}
};

template <class I, class AIteratorT, class BIteratorT>
CoordinateT<I> MergePathSearch(I diagonal, I a_len, I b_len, AIteratorT a,
                               BIteratorT b) {
  ...
  return CoordinateT<I>(std::min(x_min, a_len), // x coordinate in A
                        diagonal - x_min);      // y coordinate in B
}

template <class I, class T1, class T2, class T3>
void csrmv_merge(const bool overwrite_y, const I num_rows,
                 const I row_offsets[], const I column_indices[],
                 const T1 values[], const T2 alpha, const T3 x[],
                 I row_carry_out[], T3 value_carry_out[], T3 y[]) {

  const I *row_end_offsets = row_offsets + 1; // Merge list A: row end-offsets
  const I num_nonzeros = row_offsets[num_rows];
  int num_threads = omp_get_num_threads();
  CountingInputIterator<I> nz_indices(
      0); // Merge list B: Natural numbers(NZ indices)
  I num_merge_items = num_rows + num_nonzeros; // Merge path total length
  I items_per_thread = (num_merge_items + num_threads - 1) /
...
#pragma omp for schedule(static, 1)
    for (int tid = 0; tid < num_threads; tid++) {
      // Find starting and ending MergePath coordinates (row-idx, nonzero-idx)
      // for each thread
      I diagonal = std::min(items_per_thread * tid, num_merge_items);
      I diagonal_end = std::min(diagonal + items_per_thread, num_merge_items);
      CoordinateT<I> thread_coord = MergePathSearch(
          diagonal, num_rows, num_nonzeros, row_end_offsets, nz_indices);
      CoordinateT<I> thread_coord_end = MergePathSearch(
          diagonal_end, num_rows, num_nonzeros, row_end_offsets, nz_indices);
      // if(overwrite_y){
      ...
      for (; thread_coord.x < thread_coord_end.x; ++thread_coord.x) {
        ...
        for (; thread_coord.y < row_end_offset; ++thread_coord.y)
          running_total +=
              values[thread_coord.y] * x[column_indices[thread_coord.y]];
...
// the arrays are assumed to be on row-major order. cost O(n_vecs*nthreads)
// memory
template <class I, class T1, class T2, class T3>
void csrmv_merge_multi(const bool overwrite_y, const I num_rows,
                       const npy_intp n_vecs, const I row_offsets[],
                       const I column_indices[], const T1 values[],
                       const T2 alpha, const T3 x[], I row_carry_out[],
                       T3 value_carry_out[], T3 y[]) {
...
template <class I, class T1, class T2, class T3>
void csrmv_merge_strided(const bool overwrite_y, const I num_rows,
                         const I row_offsets[], const I column_indices[],
                         const T1 values[], const T2 alpha,
                         const npy_intp stride_x, const T3 x[],
                         I row_carry_out[], T3 value_carry_out[],
                         const npy_intp stride_y, T3 y[]) {
...
#pragma omp for schedule(static, 1)
    for (int tid = 0; tid < num_threads; tid++) {
      ...
      for (; thread_coord.x < thread_coord_end.x; ++thread_coord.x) {
        I row_end_offset = row_end_offsets[thread_coord.x];
        for (; thread_coord.y < row_end_offset; ++thread_coord.y)
          running_total += values[thread_coord.y] *
                           x[column_indices[thread_coord.y] * stride_x];

        y[thread_coord.x * stride_y] =
            alpha * running_total; // assign vs. add in-place
        running_total = 0.0;
      }
...
#pragma omp for schedule(static, 1)
    for (int tid = 0; tid < num_threads; tid++) {
      ...
      for (; thread_coord.x < thread_coord_end.x; ++thread_coord.x) {
        ...
        y[thread_coord.x * stride_y] +=
            alpha * running_total; // add in-place vs. assign
        running_total = 0.0;
      }
